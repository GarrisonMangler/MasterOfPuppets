...
local EMOTE_ENTRY_VERIFY_SECONDS = 1.50
local EMOTE_EVENT_CURRENT_SECONDS = 3.0
local EMOTE_RETRY_MIN_SECONDS = 0.75

mop.capabilities.require("mop.runtime", "2.2")
...
local should_relay_global_stop = function() return is_conductor end
...
local function stop_terminal(reason, broadcast_target_loss)
    ...
    if broadcast_target_loss then
        -- Coordination state is scoped to this authenticated run. A target-loss
        -- stop is local-band broadcast first, then relayed through the configured
        -- Chat Sync linkshell by the band conductor. A single-client run relays
        -- immediately. The host persists a run tombstone before sending chat.
        local payload = table.concat({ "2", "X", "target", local_slot }, "|")
        if coordination_available and #payload <= 96 then
            if is_conductor then mop.shared.set("m2.stop", payload) end
            mop.messages.broadcast("mirror.v2.stop", payload, PROTOCOL)
        end
        if not coordination_available or should_relay_global_stop() then
            mop.runtime.global_stop(reason)
        end
    end
    terminal = true
    clear_pending_work()
...
should_relay_global_stop = function()
    if not coordination_available then return true end
    local now = mop.runtime.shared_time()
    local elected = nil
    for slot, cid in pairs(participant_by_slot) do
        if cid ~= "0" and acknowledged[slot]
            and now - (member_last_seen[slot] or 0) <= MEMBER_SILENCE_SECONDS
            and (elected == nil or slot < elected) then elected = slot end
    end
    return elected == nil or elected == local_slot
end
...
local headgear_module = module("headgear", "h")
...
local function apply_fallback_selection(payload)
    ...
    owner.pending = {
        ...
        verify_deadline = owner == emote_module
            and mop.runtime.shared_time() + EMOTE_ENTRY_VERIFY_SECONDS or nil,
...
reconcile_desired_from_target = function()
    ...
    local headgear_visible = target_state.is_headgear_visible
    set_desired(headgear_module, headgear_visible and "1" or "0",
        function() return mop.actions.headgear_visible(headgear_visible) end,
        function(state) return state.is_headgear_visible == headgear_visible end)
...
local function action_kind(action_type)
    ...
    if action_type == 5 then return "general_action" end
...
local emote_ready_epoch = -1
local emote_reset = nil
local last_target_emote_id = 0
local last_target_emote_at = -math.huge
local seen_remote_resync = {}
local seen_remote_resync_order = {}
...
local function desired_emote_is_current(id, persistent)
    if target_state == nil or id <= 0 then return false end
    if target_state.emote_id == id then
        return not persistent or target_state.is_emote_looping
    end
    return not persistent and last_target_emote_id == id
        and mop.runtime.shared_time() - last_target_emote_at <= EMOTE_EVENT_CURRENT_SECONDS
end

local function emote_is_reset(state)
    return state ~= nil and state.emote_id == 0 and not state.is_emote_looping
end

local function begin_emote_reset(restart)
    if emote_barrier == nil or local_is_target
        or not mask_has(emote_barrier.mask, local_slot) then return end
    emote_module.pending = nil
    emote_reset = {
        epoch = emote_barrier.epoch,
        revision = emote_barrier.revision,
        restart = restart,
        attempts = 0,
        next_try = mop.runtime.shared_time(),
        reported = false,
...
local function tick_emote_readiness()
    if emote_barrier == nil or emote_barrier.phase ~= "P" or local_is_target
        or not mask_has(emote_barrier.mask, local_slot)
        or emote_ready_epoch == emote_barrier.epoch then return end
    if self_state ~= nil and self_state.is_loaded and not self_state.is_dead
        and not self_state.is_moving
        and desired_emote_is_current(emote_barrier.id, emote_barrier.persistent) then
        if send_message("mirror.v2.emote", table.concat({
                "2", "R", emote_barrier.epoch, local_slot,
            }, "|")) then
            emote_ready_epoch = emote_barrier.epoch
        end
    end
end

local function tick_emote_reset(now)
    local reset = emote_reset
    if reset == nil or emote_barrier == nil
        or reset.epoch ~= emote_barrier.epoch
        or reset.revision ~= emote_barrier.revision then
        emote_reset = nil
        return
    end
    if emote_is_reset(self_state) then
        if reset.restart and not reset.reported then
            reset.reported = send_message("mirror.v2.emote", table.concat({
                "2", "Z", reset.epoch, local_slot,
            }, "|"))
        end
        if not reset.restart or reset.reported then emote_reset = nil end
        return
    end
    if now < reset.next_try then return end
    reset.attempts = reset.attempts + 1
    mop.actions.stop_emote()
    reset.next_try = now + backoff(reset.attempts)
end
...
local function evaluate_emote_barrier()
    ...
    elseif emote_barrier.phase == "G" then
        ...
        if complete then
            ...
            if failed then emote_barrier.reset = {} end
            publish_emote_phase(emote_barrier)
            emote_barrier.retry_round = (emote_barrier.retry_round or 0) + 1
            emote_barrier.retry_not_before = nil
            if failed and not emote_barrier.remote_resync_sent then
                -- One linkshell resync aligns the other physical PCs. Later
                -- retries stay inside each band's ordered barrier instead of
                -- amplifying one failure into a cross-PC chat storm.
                emote_barrier.remote_resync_sent = true
                mop.runtime.broadcast_emote_resync(
                    emote_barrier.id,
                    emote_barrier.persistent,
                    emote_barrier.target_id)
            end
        end
    elseif emote_barrier.phase == "F" then
        local all_reset = true
        for candidate = 0, 31 do
            if mask_has(emote_barrier.mask, candidate)
                and not emote_barrier.reset[candidate] then all_reset = false end
        end
        if all_reset then
            if not desired_emote_is_current(emote_barrier.id, emote_barrier.persistent) then
                emote_barrier.phase = "S"
                publish_emote_phase(emote_barrier)
                return
            end
            local now = mop.runtime.shared_time()
            if emote_barrier.retry_not_before == nil then
                emote_barrier.retry_not_before = now
                    + math.max(EMOTE_RETRY_MIN_SECONDS, backoff(emote_barrier.retry_round or 1))
                return
            end
            if now < emote_barrier.retry_not_before then return end
            emote_barrier.epoch = next_epoch()
            emote_barrier.phase = "P"
            emote_barrier.ready = {}
            emote_barrier.outcomes = {}
            emote_barrier.reset = {}
            emote_barrier.retry_not_before = nil
            emote_ready_epoch = -1
            publish_emote_phase(emote_barrier)
...
local function start_emote_barrier(id, persistent, target_id)
    if not is_conductor then return end
    last_target_emote_id = id
    last_target_emote_at = mop.runtime.shared_time()
...
local function accept_emote_phase(payload)
    ...
    local is_new_epoch = emote_barrier == nil or epoch > emote_barrier.epoch
    local previous_epoch = emote_barrier and emote_barrier.epoch or -1
    local previous_revision = emote_barrier and emote_barrier.revision or -1
    local previous_phase = emote_barrier and emote_barrier.phase or ""
    ...
    emote_barrier.ready = is_new_epoch and {} or emote_barrier.ready or {}
    emote_barrier.outcomes = is_new_epoch and {} or emote_barrier.outcomes or {}
    emote_barrier.reset = is_new_epoch and {} or emote_barrier.reset or {}
    ...
    elseif emote_barrier.phase == "G" then
        ...
        if result_ok(result) then
            emote_module.pending = {
                ...
                attempts = 1,
                next_try = math.huge,
                verify_deadline = mop.runtime.shared_time() + EMOTE_ENTRY_VERIFY_SECONDS,
                execute = function() return result end,
                verify = function(state)
                    return state.emote_id == id and (not persistent or state.is_emote_looping)
                end,
    ...
    elseif emote_barrier.phase == "F" then
        if previous_epoch ~= epoch
            or previous_revision ~= emote_barrier.revision
            or previous_phase ~= "F" then
            begin_emote_reset(true)
        end
    elseif emote_barrier.phase == "S" then
        if previous_epoch ~= epoch
            or previous_revision ~= emote_barrier.revision
            or previous_phase ~= "S" then
            begin_emote_reset(false)
        end
    end
end
...
local function conductor_emote_message(fields, sender_content_id)
    ...
    elseif fields[2] == "O" and emote_barrier.phase == "G" then
        ...
        evaluate_emote_barrier()
    elseif fields[2] == "Z" and emote_barrier.phase == "F" then
        emote_barrier.reset[slot] = true
        evaluate_emote_barrier()
    end
end
...
local function supersede_emote_if_stopped(previous, current)
    ...
    if (previous.is_emote_looping and not current.is_emote_looping
        or previous.emote_id > 0 and current.emote_id == 0)
        and emote_barrier.phase ~= "S" then
        last_target_emote_id = 0
        last_target_emote_at = -math.huge
        emote_module.revision = emote_module.revision + 1
        emote_module.key = "0"
        emote_module.pending = nil
        emote_barrier = {
...
local function accept_remote_emote_resync(data)
    if not is_conductor or data == nil
        or text(data.run_id) ~= text(runtime_info.run_id) then return end
    local message_id = text(data.message_id)
    if message_id == "" or seen_remote_resync[message_id] then return end
    seen_remote_resync[message_id] = true
    seen_remote_resync_order[#seen_remote_resync_order + 1] = message_id
    if #seen_remote_resync_order > 32 then
        seen_remote_resync[table.remove(seen_remote_resync_order, 1)] = nil
    end

    local id = number(data.emote_id)
    local persistent = boolean(data.persistent)
    if not desired_emote_is_current(id, persistent) then return end
    if emote_barrier == nil or emote_barrier.id ~= id
        or emote_barrier.persistent ~= persistent then
        emote_module.revision = emote_module.revision + 1
        emote_module.key = tostring(id) .. ":" .. (persistent and "1" or "0")
        emote_barrier = {
            epoch = next_epoch(), revision = emote_module.revision, id = id,
            persistent = persistent, target_id = object_id(data.target_id),
            mask = acknowledged_mask(), phase = "F", ready = {}, outcomes = {},
            reset = {}, remote_resync_sent = true,
            retry_round = 1, retry_not_before = nil,
        }
    elseif emote_barrier.phase ~= "F" then
        emote_barrier.epoch = next_epoch()
        emote_barrier.phase = "F"
        emote_barrier.ready = {}
        emote_barrier.outcomes = {}
        emote_barrier.reset = {}
        emote_barrier.remote_resync_sent = true
        ...
        if number(message.schema_version) == PROTOCOL then
            ...
            if message.topic == "mirror.v2.stop"
                ...
                if is_conductor then
                    set_shared("m2.stop", message.payload)
                    mop.runtime.global_stop("target loss reported by local band participant")
                end
...
local function process_event(event)
    ...
    if event.name == "mirror.remote-emote-resync" then
        accept_remote_emote_resync(data)
        return
    end
    ...
    if event.name == "emote.played" then
        ...
        if source == launch.entity_id and number(data.emote_id) > 0 then
            last_target_emote_id = number(data.emote_id)
            last_target_emote_at = mop.runtime.shared_time()
...
while not mop.runtime.cancelled() and not terminal do
    ...
    elseif emote_module.pending ~= nil
        and emote_module.pending.verify_deadline ~= nil
        and now >= emote_module.pending.verify_deadline then
        emote_module.pending = nil
        if emote_barrier ~= nil and emote_barrier.phase == "G" then
            send_message("mirror.v2.emote", table.concat({
                "2", "O", emote_barrier.epoch, local_slot, "fail",
            }, "|"))
        end

-- Command-free, event-driven target mirror. Every mirrored operation uses a
-- typed Master Of Puppets game function and is verified against the local
-- actor watch. Missing exact jobs/cosmetics/emotes are left unchanged.
...
if not target_name or target_name == "" then
    error("Mirror Target Combat could not resolve the initiator or selected target")
end
...
local function state_from(value)
    ...
    return {
        ...
        emote_target_game_object_id = object_id(value.emote_target_game_object_id),
...
local function reveal_explicit_target()
    ...
    -- Copy the conductor's native target directly so visibility plugins can
    -- reveal the actor without emitting any chat command.
    mop.actions.target_of(conductor_name)
end
...
-- Freeze the synchronized roster's visible actor IDs once. Puppets remain
-- mutually visible even on clients that hide unrelated players, so this lets
-- target mirroring distinguish a roster member without repeated world scans.
local roster_actor_ids = {}
for _, participant in ipairs(mop.participants.list()) do
    if participant.actor then
        roster_actor_ids[object_id(participant.actor.game_object_id)] = true
        roster_actor_ids[object_id(participant.actor.entity_id)] = true
    end
end
...
local next_reveal = 0
local next_reconcile = mop.time() + 0.75
...
local function exact_use(kind, id, persistent, target_id)
    ...
    local result
    if target_id and object_id(target_id) ~= "0" then
        if persistent == nil then
            result = mop.actions.use_exact_on(kind, id, object_id(target_id))
        else
            result = mop.actions.use_exact_on(kind, id, object_id(target_id), persistent)
        end
    else
        if persistent == nil then
    ...
    if not result.ok then
        if unsupported_result(result) then unavailable[key] = true end
...
local function request_emote_stop(reason, force)
    ...
    local has_observed_emote = self_state
        and (self_state.is_emote_looping or self_state.emote_id > 0)
    ...
    if not pending_emote_stop or now >= pending_emote_stop.next_try then
        local result = report("emote stop (" .. reason .. ")", 0, mop.actions.stop_emote())
        if result and result.ok then mirrored_persistent_emote_id = nil end
        pending_emote_stop = {
            attempts = (pending_emote_stop and pending_emote_stop.attempts or 0) + 1,
            next_try = now + 0.10,
            deadline = now + 1.0,
        }
    end
...
local function desired_target_id()
    local selected = object_id(target_state and target_state.target_game_object_id)
    -- Never retarget one puppet onto another puppet. If the mirrored actor is
    -- targeting anyone in this synchronized roster, keep the mirrored actor
    -- selected instead.
    if selected ~= "0" and not roster_actor_ids[selected] then return selected end
    return object_id(target_state and target_state.game_object_id)
end

local function request_target(id)
    if local_is_target or id == "0" then return end
    local result = mop.actions.target(id)
    if not result.ok then
        -- Hidden Wind-up/Kazuko clients copy the visible band leader's native
        -- target. The function refuses to copy a leader targeting another ID.
        result = mop.actions.target_via_leader(id)
    end
    report("selected target", id, result)
    pending_target = {
        id = id,
        attempts = pending_target and pending_target.id == id and pending_target.attempts + 1 or 1,
        next_try = mop.time() + 0.35,
        deadline = mop.time() + 10.0,
    }
end
...
local function request_job(id)
    ...
    local result = report("class/job", id, mop.actions.job(id))
...
local function request_weapon(drawn)
    if self_state and (self_state.is_emote_looping or self_state.emote_id > 0
...
local function request_jump(sequence)
    ...
    if self_state.is_emote_looping or self_state.emote_id > 0
        or mirrored_persistent_emote_id ~= nil then
        request_emote_stop("jump", true)
        pending_jump = pending_jump or { sequence = sequence, baseline = self_state.jump_sequence, attempts = 0 }
        pending_jump.next_try = mop.time() + 0.10
        pending_jump.deadline = mop.time() + 0.80
        return
    end
    report("jump", 2, mop.actions.use("general_action", 2, "local"))
    pending_jump = {
        sequence = sequence,
        baseline = pending_jump and pending_jump.baseline or self_state.jump_sequence,
        attempts = pending_jump and pending_jump.sequence == sequence and pending_jump.attempts + 1 or 1,
        next_try = mop.time() + 0.10,
        deadline = mop.time() + 0.80,
    }
end

local function request_sprint()
    if local_is_target or (self_state and self_state.is_sprinting) then return end
    report("sprint", 3, mop.actions.use("general_action", 3, "local"))
    pending_sprint = {
...
local function reconcile()
    ...
    local target_id = desired_target_id()
    if object_id(self_state.target_game_object_id) ~= target_id
        and (not pending_target or mop.time() >= pending_target.next_try) then
    ...
    if self_state.mount_id ~= target_state.mount_id then
        if target_state.mount_id > 0 then exact_use("mount", target_state.mount_id, nil, nil)
    ...
    if self_state.companion_id ~= target_state.companion_id then
        if target_state.companion_id > 0 then exact_use("minion", target_state.companion_id, nil, nil)
    ...
    if self_state.ornament_id ~= target_state.ornament_id then
        if target_state.ornament_id > 0 then exact_use("fashion_accessory", target_state.ornament_id, nil, nil)
    ...
    if self_state.facewear_id ~= target_state.facewear_id then
        if target_state.facewear_id > 0 then exact_use("facewear", target_state.facewear_id, nil, nil)
    ...
    if target_wants_loop
        ...
        local emote_target = target_state.emote_target_game_object_id
        if object_id(emote_target) == target_state.game_object_id then emote_target = nil end
        exact_use("emote", target_state.emote_id, true, emote_target)
    end
...
local function apply_self(next_state)
    ...
    if previous and ((previous.is_dead and not self_state.is_dead)
        ...
        pending_job, pending_weapon, pending_target, pending_pose, pending_status = nil, nil, nil, nil, nil
    ...
    if pending_emote_stop and mirrored_persistent_emote_id == nil
        and not self_state.is_emote_looping
        and self_state.emote_id == 0 then pending_emote_stop = nil end
    if self_state.is_moving and (self_state.is_emote_looping
        or self_state.emote_id > 0 or mirrored_persistent_emote_id ~= nil) then
        request_emote_stop("local movement safety", true)
    end
    if pending_target and object_id(self_state.target_game_object_id) == pending_target.id then pending_target = nil end
...
local function apply_target(next_state, found)
    ...
    if previous and target_state.emote_id > 0
        ...
        local emote_target = target_state.emote_target_game_object_id
        if object_id(emote_target) == target_state.game_object_id then emote_target = nil end
        exact_use("emote", target_state.emote_id, false, emote_target)
    end
    reconcile()
end
...
local function run_pending_combat()
    ...
    elseif action.target_id ~= "0" and tonumber(action.target_id) ~= target_state.entity_id then
        result = mop.actions.use_on(action.kind, action.id, action.target_id)
    else
...
local function process_retries()
    ...
    if pending_jump and now >= (pending_jump.next_try or 0) then
        if now >= (pending_jump.deadline or now + 1) or pending_jump.attempts >= 6 then
            mop.log("Unable to verify mirrored jump")
            pending_jump = nil
    ...
    if pending_target and now >= pending_target.next_try then
        if now >= pending_target.deadline or pending_target.attempts >= 24 then
            mop.log("Unable to acquire mirrored target " .. pending_target.id .. " directly or from the band leader")
            pending_target = nil
        else request_target(pending_target.id) end
    end
...
while mop.is_running() do
    ...
    if event.status == "event" then
        ...
        elseif event.name == "emote.played" then
            ...
            elseif target_state and source_entity_id == target_state.entity_id
                ...
                local emote_target = object_id(event.data.target_id)
                if emote_target == target_state.game_object_id
                    or tonumber(emote_target) == target_state.entity_id then
                    emote_target = nil
                end
                exact_use("emote", emote_id, as_bool(event.data.is_persistent), emote_target)
            end
        elseif event.name == "combat.action" then
            local source_entity_id = tonumber(event.data.source_entity_id or "0") or 0
    ...
    process_retries()
    local now = mop.time()
    if target_lost and now >= next_reveal then
        reveal_explicit_target()
        next_reveal = now + 0.5
    end
    if force_reconcile or now >= next_reconcile then
        local refreshed_self = mop.self.watch()
        if refreshed_self.status == "found" then apply_self(state_from(refreshed_self.actor)) end

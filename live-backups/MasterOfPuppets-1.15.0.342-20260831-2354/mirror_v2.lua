...
mop.capabilities.require("mop.runtime", "2.0")
...
local local_slot = tonumber(mop.get_slot()) or 0
...
local function stop_terminal(reason, broadcast_target_loss)
    ...
    terminal = true
    clear_pending_work()
    mop.log("Mirror v2 terminal: " .. reason)
...
local launch = {
    name = text(mop.get_var("mop_run_target_launch_name")),
    game_object_id = object_id(mop.get_var("mop_run_target_launch_game_object_id")),
    entity_id = number(mop.get_var("mop_run_target_launch_entity_id")),
    content_id = text(mop.get_var("mop_run_target_launch_content_id")),
    source = text(mop.get_var("mop_run_target_launch_source")),
}
...
if not local_is_target then
    ...
    member_last_seen[local_slot] = mop.time()
end
...
local function apply_fallback_selection(payload)
    ...
    owner.pending = {
        revision = owner.revision,
        attempts = 1,
        next_try = mop.time() + RETRY_MIN_SECONDS,
...
local function accept_emote_phase(payload)
    ...
    elseif emote_barrier.phase == "F" then
        ...
        if EMOTE_RESET_VERIFIED then
            mop.log("Mirror v2 emote reset integration is enabled")
        end
...
local function admit_member(slot)
    ...
    member_last_seen[slot] = mop.time()
...
local next_reconcile = mop.time() + RECONCILE_SECONDS
local next_hello = mop.time() + MEMBER_HELLO_SECONDS

mop.log("Mirror v2 started for immutable local target " .. launch.name
    .. " (" .. launch.game_object_id .. ")")

while mop.is_running() and not terminal do
